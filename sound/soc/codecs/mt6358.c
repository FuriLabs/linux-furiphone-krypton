			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		dev_dbg(priv->dev, "%s(), set vow disable at the last vow capture widget\n",
			 __func__);
		priv->vow_enable = 0;
		break;
	default:
...
static int mt6358_vow_cfg_enable(struct mt6358_priv *priv)
{
	...
	dev_dbg(priv->dev, "%s(), mic_type: %d, vow_dmic_lp: %d\n", __func__,
...
static int mt6358_vow_amic_enable(struct mt6358_priv *priv)
{
	...
	regmap_update_bits(priv->regmap, MT6358_AUDENC_ANA_CON2,
			   0xC31F, 0xC31F);
	dev_dbg(priv->dev, "%s(), mt_vow_aud_lpw_enable 0xC31F\n",
		 __func__);
...
static int mt6358_vow_amic_disable(struct mt6358_priv *priv)
{
	...
	regmap_update_bits(priv->regmap, MT6358_AUDENC_ANA_CON2,
			   0xC31F, 0x0000);
	dev_dbg(priv->dev, "%s(), mt_vow_aud_lpw_disable 0x0\n",
		 __func__);
	/* L ADC input sel : off, disable L ADC */
	regmap_update_bits(priv->regmap, MT6358_AUDENC_ANA_CON0,
			   0xf000, 0x0000);
...
static void mt6358_parse_dt(struct mt6358_priv *priv)
{
	...
	if (ret) {
		dev_dbg(dev, "%s() failed to read mic-type, default ACC\n",
			 __func__);

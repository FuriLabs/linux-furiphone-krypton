...
/*
 *  mt6873-afe-clk.c  --  Mediatek 6873 afe clock ctrl
 *
...
int mt6873_set_audio_int_bus_parent(struct mtk_base_afe *afe,
				    int clk_id)
{
...
int mt6873_afe_enable_clock(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
	...
	ret = mt6873_set_audio_int_bus_parent(afe,
					      CLK_CLK26M);
...
void mt6873_afe_disable_clock(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
...
int mt6873_afe_dram_request(struct device *dev)
{
...
int mt6873_afe_dram_release(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
	struct arm_smccc_res res;
...
int mt6873_apll1_enable(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
void mt6873_apll1_disable(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
...
int mt6873_apll2_enable(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
void mt6873_apll2_disable(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
...
int mt6873_get_apll_rate(struct mtk_base_afe *afe, int apll)
{
	return (apll == MT6873_APLL1) ? 180633600 : 196608000;
}

int mt6873_get_apll_by_rate(struct mtk_base_afe *afe, int rate)
{
	return ((rate % 8000) == 0) ? MT6873_APLL2 : MT6873_APLL1;
}

int mt6873_get_apll_by_name(struct mtk_base_afe *afe, const char *name)
{
...
void mt6873_mck_disable(struct mtk_base_afe *afe, int mck_id)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
	int m_sel_id = mck_div[mck_id].m_sel_id;
	int div_clk_id = mck_div[mck_id].div_clk_id;
...
int mt6873_init_clock(struct mtk_base_afe *afe)
{
	struct mt6873_afe_private *afe_priv = afe->platform_priv;
	int i = 0;

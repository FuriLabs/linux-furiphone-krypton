 ...
 *  15072011511229: add ??log??,??log?????????log??
...
static struct imgsensor_info_struct imgsensor_info = {
  ...
  .custom5 = {
	...
	.i2c_addr_table = {0x20, 0x5a, /*0x20,*/0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
...
static void check_output_stream_on(void)
{
	...
	{		
		...
		if(read_count == 4)			
			pr_info("[error][0x0005, 0xff]sensor not output\n");	
	}
}
					...
					MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...

	{
	    ...
	    of_node = of_find_compatible_node(NULL, NULL, "mediatek,imgsensor");
	    if (0 == of_property_read_u8(of_node, "s5k3l8_mipi_raw_mirror", &ret)) {
			pr_info("[qyTag] %s: read mirror:%d\n", __func__, ret);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
			//pr_info("SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY scenarioId:%d\n", *feature_data);

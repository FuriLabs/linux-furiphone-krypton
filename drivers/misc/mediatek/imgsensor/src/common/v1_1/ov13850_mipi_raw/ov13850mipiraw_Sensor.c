...
/*capture_firsts_flag & pre_currefps for resolve ov13850r2a 
* low light zsd capture color-line issue
*/
int capture_firsts_flag = 0;
...
{
	capture_firsts_flag = 0;
	pre_currefps = currefps;
}
else
{
	capture_firsts_flag = 1;
}
if (capture_firsts_flag == 0)
{
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("ov13850 get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("ov13850 get_imgsensor_id Read sensor id fail, write id: 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                printk("ov13850 open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("ov13850 open  Read sensor id fail, write id: 0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)

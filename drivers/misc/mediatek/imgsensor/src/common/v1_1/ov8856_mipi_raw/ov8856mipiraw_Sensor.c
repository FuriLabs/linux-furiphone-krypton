...
static struct imgsensor_info_struct imgsensor_info = {
 ...
 .pre = {
	...
	.i2c_addr_table = {0x6c, 0x6d, 0x42, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk(
				 "ov8856:i2c write id: 0x%x, sensor id: 0x%x, ver = 0x%x<0xB1=r2a,0xB0=r1a>\n",
				imgsensor.i2c_write_id,
				 *sensor_id, read_cmos_sensor(0x302A));
				printk("ov8856:hct_add %s id 0x%x get ok ID 0x%x\n",__func__,*sensor_id,OV8856_SENSOR_ID);
				return ERROR_NONE;
			}
			printk("ov8856:Read sensor id fail,write_id:0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			printk("ov8856:hct_add %s id 0x%x\n",__func__,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if (sensor_id == imgsensor_info.sensor_id) {				
				...
				printk("hct_add ok %s id 0x%x\n",__func__,sensor_id);
				break;
			}
			LOG_INF("Read sensor id fail, write: 0x%x, sensor: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			printk("hct_add %s id 0x%x\n",__func__,sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)

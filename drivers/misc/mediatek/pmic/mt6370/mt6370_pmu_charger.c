			      ...
			      struct mt6370_pmu_charger_data *chg_data, bool en)
{
	...
	if (en && mt6370_is_meta_mode(chg_data)) {
		...
		dev_dbg(chg_data->dev, "force Standard USB Host in meta\n");
...
dcd_timeout:
	...
	if (chg_data->psy_usb_type != POWER_SUPPLY_USB_TYPE_DCP) {
		...
		if (ret < 0)
			dev_dbg(chg_data->dev, "%s: disable chgdet fail\n",
				   __func__);
	}
...
out:
	...
	if (ret < 0)
		dev_dbg(chg_data->dev, "%s: fail, en = %d\n", __func__, en);
	mutex_unlock(&chg_data->ichg_access_lock);
	mt6370_power_supply_changed(chg_data);
	return ret;
}
...
static int mt6370_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
	...
	if (!chg_data->psy) {
		dev_dbg(chg_data->dev, "%s: cannot get psy\n", __func__);
		return -ENODEV;
	}
	...
	struct mt6370_pmu_charger_data *chg_data)
{
	...
	if (of_property_read_u32(np, "bc12_sel", &chg_desc->bc12_sel) < 0)
		dev_dbg(chg_data->dev, "%s: no bc12_sel\n", __func__);
...
static void mt6370_get_bootmode(struct mt6370_pmu_charger_data *chg_data)
{
	...
	if (!boot_node) {
		dev_dbg(chg_data->dev, "%s: get bootmode phandle fail\n",
					  __func__);
		return;
	}
	...
	if (!tag) {
		dev_dbg(chg_data->dev, "%s: get property atag,boot fail\n",
					  __func__);
		return;
	}
...
static int mt6370_chg_init_setting(struct mt6370_pmu_charger_data *chg_data)
{
	...
	if (ret < 0)
		dev_dbg(chg_data->dev, "%s disable dcd fail\n", __func__);
#endif
				   ...
				   const char *buf, size_t count)
{
	...
	if (ret < 0) {
		dev_dbg(dev, "set passcode1 fail\n");
		return ret;
	}
	ret = mt6370_pmu_reg_write(chg_data->chip,
				   MT6370_PMU_REG_RSTPASCODE2, 0x96);
	if (ret < 0) {
		dev_dbg(dev, "set passcode2 fail\n");
		return ret;
	}
	...
	if (ret < 0) {
		dev_dbg(dev, "set reset bits fail\n");
		return ret;
	}
	...
	ret = mt6370_pmu_reg_clr_bit(chg_data->chip,
		MT6370_PMU_REG_CHGHIDDENCTRL15, 0x01);
	if (ret < 0) {
		dev_dbg(dev, "set auto sensing disable\n");
		return ret;
	}
	mt6370_enable_hidden_mode(chg_data, false);
	mdelay(50);
	...
	if (ret < 0) {
		dev_dbg(dev, "enter shipping mode\n");
		return ret;
	}
	return count;
}

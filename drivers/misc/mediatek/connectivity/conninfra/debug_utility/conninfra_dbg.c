...
static int conninfra_dbg_set_platform_config(int par1, int par2, int par3)
{
        ...
        pr_debug("set platform config %d\n", par2);
        return 0;
}
...
static int conninfra_dbg_clk_reg_read(int par1, int par2, int par3)
{
	...
	if (map == NULL) {
		pr_debug("%s clock ic regmap is NULL.\n", __func__);
		return 0;
	}
...
static int conninfra_dbg_spi_read(int par1, int par2, int par3)
{
	...
	if (par2 < 0 || par2 >= SYS_SPI_MAX) {
		pr_debug("%s par2 is out of range\n", __func__);
		return 0;
	}

	spi_ret = conninfra_spi_read(par2, par3, &data);
	if (spi_ret == 0) {
		pr_debug("%s read[%s]addr[0x%x]val[0x%x] ok\n",
		...
		pr_debug("%s read[%s]addr[0x%x] failed(%d)\n",
	...
	if (iRet)
		pr_debug("[%s] string error, iRet = %d", __func__, iRet);

	get_lock_ret = osal_lock_sleepable_lock(&g_dump_lock);
	if (get_lock_ret) {
		pr_debug("[%s] dump lock fail, ret=%d", __func__, get_lock_ret);
		return 0;
	}
...
static int conninfra_dbg_mcu_log_ctrl(int par1, int par2, int par3)
{
	...
	ret = conninfra_core_force_conninfra_wakeup();
	if (ret) {
		pr_debug("[%s] conninfra wakup fail\n", __func__);
		return -1;
	}
	...
	ret = conninfra_core_force_conninfra_sleep();
	if (ret) {
		pr_debug("[%s] conninfra sleep fail\n", __func__);
		return -1;
	}

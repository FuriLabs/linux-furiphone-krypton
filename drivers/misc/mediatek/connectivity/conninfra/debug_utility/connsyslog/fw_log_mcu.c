...
int fw_log_mcu_register_event_cb(struct fw_log_mcu_info *info)
{
	...
	ret = connsys_log_register_event_cb(info->conn_type,
		info->mcu_event_cb);
	if (ret)
		pr_debug("[%s] Fail to register connsys_log_register_event_cb, conn_type=[%d]\n",
...
int fw_log_mcu_init(struct fw_log_mcu_info *info, const struct file_operations *ops)
{
	...
	if (ret)
		pr_debug("[%s] Fail to alloc_chrdev_region, conn_type=[%d]\n", __func__, info->conn_type);
...
error:
	...
	pr_debug("fw_log_mcu_init fail, conn_type=[%d]\n", info->conn_type);
	return -1;
}

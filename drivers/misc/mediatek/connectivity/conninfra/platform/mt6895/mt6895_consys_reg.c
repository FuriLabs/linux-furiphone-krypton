...
static void consys_print_log(const char *title, struct conn_debug_info_mt6895 *info)
{
	...
	for (i = 0; i < info->length; i++) {
		...
		if (strlen(debug_buf) + strlen(temp) < CONSYS_DUMP_BUF_SIZE)
			...
			pr_debug("%s debug_buf len is not enough\n", __func__);
	}
	pr_debug("%s\n",debug_buf);
}
...
static void consys_debug_init_mt6895(void)
{
	...
	if (debug_info == NULL) {
		pr_debug("%s debug_info malloc failed\n", __func__);
		return;
	}

	debug_buf = osal_malloc(CONSYS_DUMP_BUF_SIZE);
	if (debug_buf == NULL) {
		pr_debug("%s debug_buf malloc failed\n", __func__);
		return;
	}
...
int consys_reg_init(struct platform_device *pdev)
{
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			...
			if (ret) {
				pr_debug("Get Reg Index(%d-%s) failed",
		...
		pr_debug("[%s] can't find CONSYS compatible node\n", __func__);
		return ret;
	}

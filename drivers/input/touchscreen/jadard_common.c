...
static void jadard_wake_event_parse(struct jadard_ts_data *ts, int ts_status)
{
    ...
    printk("%s: Entering, ts_status=%d, wake_event=0x%02x\n", __func__, ts_status, wake_event);
...
#endif
            if (pjadard_ic_data->JD_X_RES <= pjadard_ic_data->JD_Y_RES) {
                input_report_abs(ts->input_dev, ABS_MT_POSITION_X, 1200 - pjadard_host_data->x[i]);
                input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 1920 - pjadard_host_data->y[i]);
            } else {
...
static int jadard_err_ctrl(uint8_t *buf, int buf_len, int ts_status)
{
    ...
    if (pjadard_ts_data->rst_active) {
        ...
        printk("[rst_active]:%s: Back from reset, ready to serve.\n", __func__);
        ts_status = JD_RST_OK;
        goto GOTO_END;
    }
...
void jadard_usb_status(int status, bool renew)
{
    ...
    if (ts->usb_status) {
        if ((status != ts->usb_connected) || renew) {
            ...
            g_module_fp.fp_usb_detect_set(ts->usb_status);
            printk("%s: Cable status update: 0x%2.2X\n", __func__, ts->usb_connected);
        }
    }
}
...
static bool jadard_auto_upgrade_check(void)
{
    ...
    if (g_module_fp.fp_read_fw_ver_bin() == JD_NO_ERR) {
        printk("pjadard_ic_data->fw_ver=%08x, g_jadard_fw_ver=%08x\n",
            pjadard_ic_data->fw_ver, g_jadard_fw_ver);
        printk("pjadard_ic_data->fw_cid_ver=%08x, g_jadard_fw_cid_ver=%08x\n",
            pjadard_ic_data->fw_cid_ver, g_jadard_fw_cid_ver);

        if ((pjadard_ic_data->fw_ver < g_jadard_fw_ver) ||
            (pjadard_ic_data->fw_cid_ver < g_jadard_fw_cid_ver)) {
            printk("Need to upgrade FW\n");
            upgrade = true;
        } else {
            printk("No need to upgrade FW\n");
        }
    }
                                ...
                                size_t len, loff_t *pos)
{
    ...
    if (len >= sizeof(buf_tmp)) {
        printk("%s: no command exceeds %ld chars.\n", __func__, sizeof(buf_tmp));
        return -EFAULT;
    }
...
int jadard_input_register(struct jadard_ts_data *ts)
{
...
#endif
#endif
    printk("input_set_abs_params: min_x %d, max_x %d, min_y %d, max_y %d\n",
    ...
    printk("%s, input device registered.\n", __func__);
...
{
    printk("event = %d, pjadard_ts_data->suspended = %d", (int)event, pjadard_ts_data->suspended);
...
static int jadard_disp_notifier_callback(struct notifier_block *nb, unsigned long event, void *v)
{
    ...
    if (ts && v) {
        printk("MTK_DRM event = %lu, blank = %d\n", event, *blank);
        switch (*blank) {
        case MTK_DISP_BLANK_UNBLANK:
            if (MTK_DISP_EARLY_EVENT_BLANK == event) {
                printk("resume: event = %lu, Skipped\n", event);
            } else if (MTK_DISP_EVENT_BLANK == event) {
                printk("resume: event = %lu, TP_RESUME\n", event);
                jadard_chip_common_resume(ts);
            }
            break;
        case MTK_DISP_BLANK_POWERDOWN:
            if (MTK_DISP_EARLY_EVENT_BLANK == event) {
                printk("suspend: event = %lu, TP_SUSPEND\n", event);
                jadard_chip_common_suspend(ts);
            } else if (MTK_DISP_EVENT_BLANK == event) {
                printk("suspend: event = %lu, Skipped\n", event);
            }
            break;
        }
    } else {
            ...
            if (((rdata[1] == 0x00) && (rdata[2] == 0x00) && (rdata[3] == 0x00)) || (same_data >= 5)) {
                if ((rdata[0] == 0xD8) || (rdata[0] == 0xDA) || (rdata[0] == 0xDC) || (rdata[0] == 0xDE) ||
                    ...
                    if (pjadard_ts_data->fw_ready == true) {
                        printk("PC(0-3):0x%02x, 0x%02x, 0x%02x, 0x%02x\n", rdata[0], rdata[1], rdata[2], rdata[3]);
                        ...
                        if (same_data >= 5) {
                            ...
                            if (bypass == 0) {
                            #ifdef JD_ZERO_FLASH
                                printk("Upgrade fw by the same pc\n");
                            ...
                            #else
                                printk("Reset Touch chip by the same pc\n");
                                jadard_int_enable(false);
                            ...
                            pjadard_ts_data->power_on_upgrade = false;
                        #else
                            printk("Reset Touch chip by fault address\n");
                            jadard_int_enable(false);
                            g_module_fp.fp_soft_reset();
                            jadard_report_all_leave_event(pjadard_ts_data);
                            jadard_int_enable(true);
...
int jadard_chip_common_suspend(struct jadard_ts_data *ts)
{
    ...
    if ((pjadard_debug != NULL) && (*pjadard_debug->fw_dump_going)) {
        printk("%s: Flash dump is going, reject suspend\n", __func__);
        return 0;
    }
    ...
    if (ts->SMWP_enable) {
        ...
        printk("[jadard] %s: SMART_WAKEUP enable\n", __func__);
        return 0;
    }
#endif
...
int jadard_chip_common_resume(struct jadard_ts_data *ts)
{
    ...
    if (!ts->suspended) {
        printk("%s: Already resumed, Skipped\n", __func__);
        return 0;
    } else {
        printk("%s: enter\n", __func__);
        ts->suspended = false;
...
#ifdef JD_ZERO_FLASH
    printk("Upgrade fw in zero flash mode\n");
#if defined(JD_SMART_WAKEUP)
    skip_fw_upgrade = ts->SMWP_enable;
#endif
    if (ts->diag_thread_active && skip_fw_upgrade) {
        printk("Diag thread is active! Skip Update with zero flash\n");

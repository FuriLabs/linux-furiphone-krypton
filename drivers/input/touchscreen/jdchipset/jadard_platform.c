                    ...
                    struct jadard_platform_data *pdata)
{
    ...
    pjadard_ic_data->printkNT_EDGE = (!ret ? (data > 0 ? true : false) : true);
    ret = of_property_read_u32(dt, "jadard,stylus", &data);
    pjadard_ic_data->JD_STYLUS_EN = (!ret ? (data > 0 ? true : false) : false);

    printk("DT:MAX_PT = %d, INT_IS_EDGE = %d, STYLUS_EN = %d\n", pjadard_ic_data->JD_MAX_PT,
        pjadard_ic_data->printkNT_EDGE, pjadard_ic_data->JD_STYLUS_EN);
    ...
    if (of_property_read_u32_array(dt, "jadard,panel-coords", coords, coords_size) == 0) {
        ...
        printk("DT:panel-coords = %d, %d, %d, %d\n", pdata->abs_x_min,
    ...
    printk("DT:gpio_irq = %d, gpio_rst = %d\n", pdata->gpio_irq, pdata->gpio_reset);
...
int jadard_ts_register_interrupt(void)
{
    ...
    if (client->irq) {
        ...
        if (ret == 0) {
            ts->irq_enabled = 1;
            printk("%s: irq enabled at IRQ: %d\n", __func__, client->irq);
        } else {
            JD_E("%s: request_irq failed\n", __func__);
        }
    } else {
        printk("%s: client->irq is empty.\n", __func__);
    }
...
int jadard_drm_check_dt(struct jadard_ts_data *ts)
{
    ...
    if (count <= 0) {
        printk("%s: find drm_panel count(%d) fail\n", __func__, count);
        return 0;
    }
    printk("%s: find drm_panel count(%d)\n", __func__, count);
    for (i = 0; i < count; i++) {
        ...
        of_node_put(node);
        if (!IS_ERR(panel)) {
            printk("%s: find drm_panel successfully\n", __func__);
                            ...
                            unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data &&
        ...
        ts != NULL && ts->dev != NULL) {
        blank = evdata->data;
        printk("MSM_DRM event:%lu, blank:%d\n", event, *blank);
...
{
    printk("I2C Jadard kmodule common touch panel driver init\n");
...
{
    printk("I2C Jadard common touch panel driver init\n");
...
{
    printk("I2C jadard_workarround_init by Driver\n");

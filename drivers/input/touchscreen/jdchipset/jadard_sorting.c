...
{
#if 1 //defined(__JADARD_GKI__)
    printk("Cancel write result file for GKI\n");
    ...
    JD_D("%s: Entering!\n", __func__);
    printk("data size=0x%04X\n", jd_g_rslt_data_len);
    printk("Ouput result to %s\n", filepath);
...
static void jadard_sorting_data_deinit(void)
{
    ...
    if (jd_g_sorting_threshold) {
        ...
        printk("Free the jd_g_sorting_threshold\n");
    }
...
static void jadard_pst_enter_slpin_mode_bymppw(void)
{
    ...
    printk("Start to Enter Sleep In\n");
    ...
    if (LCD_Status != JD_LCD_STATUS_SLEEP_IN) {
        printk("Enter Sleep In Fail\n");
    } else {
        printk("Enter Sleep In Finished\n");
    }
...
static void jadard_pst_enter_slpout_mode_bymppw(void)
{
    ...
    printk("Start to Enter Sleep Out\n");
    ...
    if (LCD_Status != JD_LCD_STATUS_SLEEP_OUT) {
        printk("Enter Sleep Out Fail\n");
    } else {
        printk("Enter Sleep Out Finished\n");
    }
...
#endif
            printk("Switch SortMode Check: %d, not equal %d\n", *enter_flag, PST_MP_Mode_Index);
...
{
    printk("Start to Enter LPWUG Idle Diff Mode\n");
...
#endif
        printk("Retry LPWUG Idle Diff Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter LPWUG Idle Diff Mode Finished\n");
    }
...
{
    printk("Start to Enter LPWUG Idle Mode\n");
...
#endif
        printk("Retry LPWUG Idle Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter LPWUG Idle Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to LPWUG Active Diff Mode\n");
...
#endif
        printk("Retry LPWUG Active Diff Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter LPWUG Active Diff Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to LPWUG Active Mode\n");
...
#endif
        printk("Retry LPWUG Active Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter LPWUG Active Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to Enter Normal Idle Diff Mode\n");
...
#endif
        printk("Retry Normal Idle Diff Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter Normal Idle Diff Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to Enter Normal Idle Mode\n");
...
#endif
        printk("Retry Normal Idle Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter Normal Idle Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to Enter Normal Active Diff Mode\n");
...
#endif
        printk("Retry Normal Active Diff Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter Normal Active Diff Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to Enter Normal Active Mode\n");
...
#endif
        printk("Retry Normal Active Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter Normal Active Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to Enter Short Mode\n");
...
#endif
        printk("Retry Short Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter Short Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
{
    printk("Start to Enter Open Mode\n");
...
#endif
        printk("Retry Open Mode Switching\n");
        SocReset_Flag = true;
    } else {
        printk("Enter Open Mode Finished\n");
    }
    g_module_fp.fp_ClearMpBypassMain();
}
...
static void jadard_Sorting_BusyStatus_Check(void)
{
    ...
    printk("Start Sorting_BusyStatus_Check\n");
...
#endif
            printk("MPAP_HandShake_Status: 0x%02X\n", pStatus);
            return;
        } else {
            SocReset_Flag = false;
        }
    ...
    printk("Sorting_BusyStatus_Check Pass\n");
}
...
static void jadard_Start_to_Get_DiffData_Function(void)
{
    ...
    if (SocReset_Flag == true) {
...
#endif
        printk("Retry Diff Data Reading\n");
    } else {
        ...
        printk("Get Diff Data Finished\n");
    }
}

static void jadard_Start_to_Get_RawData_Function(void)
{
    printk("Start to Get Raw Data\n");
...
#endif
        printk("Retry Raw Data Reading\n");
    } else {
        printk("Get Raw Data Finished\n");
    }
}
...
static void jd_test_data_get(uint8_t *RAW, char *start_log, char *result, char *cnt_log, int now_item)
{
    ...
    /* for(i = 0; i < len; i++)
    {
        printk("0x%04X, ", jd_g_rslt_data[i + jd_g_rslt_data_len]);
        if(i > 0 && (i % 16 == 15))
            printk("\n");
    } */
...
static int jadard_parse_sorting_threshold_file(void)
{
    ...
    JD_D("%s,Entering \n", __func__);
    printk("sorting threshold file name = %s\n", file_name);
    ...
    printk("data_lines=%d ,line_max_len=%d\n", data_lines, line_max_len);
    printk("file_size=%d\n", (int)file_entry->size);
...
static int jadard_read_sorting_threshold(void)
{
    ...
    printk("JD_SORTING_ACTIVE_ITEM(%d) JD_READ_THRESHOLD_SIZE(%d)\n", JD_SORTING_ACTIVE_ITEM, JD_READ_THRESHOLD_SIZE);
    ...
    for (i = 0; i < JD_READ_THRESHOLD_SIZE; i = i + 2) {
        ...
        for (j = 0; j < pjadard_ic_data->JD_X_NUM * pjadard_ic_data->JD_Y_NUM; j++) {
            if (j % pjadard_ic_data->JD_Y_NUM == 0) {
                ...
                printk("%d:%d ", jd_g_sorting_threshold[i][j], jd_g_sorting_threshold[i + 1][j]);
            }
        }
    }
#endif
...
static int jadard_sorting_test(char *json_buf, size_t len)
{
...
#ifdef JD_UPGRADE_ITO_FW
    printk("%s: Bin file name(%s)\n", __func__, fileName);

#ifdef JD_ZERO_FLASH
    printk("Running Zero flash upgrade\n");
    ...
    if (fw_ret < 0) {
        ...
        printk("Zero flash upgrade success\n");
    }
#else
    printk("Running flash upgrade\n");

...
static int jd9366tp_Read_BackDoor_RegSingle(uint32_t addr, uint8_t *rdata)
{
    ...
    ReCode = jadard_bus_read(addrBuf, sizeof(addrBuf), readBuf, sizeof(readBuf), JD9366TP_BUS_RETRY_TIMES);
    *rdata = readBuf[0];
...
static int jd9366tp_Write_BackDoor_RegSingle(uint32_t addr, uint8_t wdata)
{
    ...
    return jadard_bus_write(addrBuf, sizeof(addrBuf), writeBuf, sizeof(writeBuf), JD9366TP_BUS_RETRY_TIMES);
}
...
static int jd9366tp_Read_BackDoor_RegMulti(uint32_t addr, uint8_t *rdata, uint16_t rlen)
{
    ...
    return jadard_bus_read(addrBuf, sizeof(addrBuf), rdata, rlen, JD9366TP_BUS_RETRY_TIMES);
}
...
static int jd9366tp_Write_BackDoor_RegMulti(uint32_t addr, uint8_t *wdata, uint16_t wlen)
{
    ...
    return jadard_bus_write(addrBuf, sizeof(addrBuf), wdata, wlen, JD9366TP_BUS_RETRY_TIMES);
}
...
#if !defined(printk2C_SINGLE_MODE) && !defined(CONFIG_JD_HID)
...
static int jd9366tp_read_slave_by_master(uint32_t addr, uint8_t *rdata, uint16_t rlen)
{
    ...
    if (rlen > 0) {
        printk("%s: Start read slave ram addr: 0x%08x\n", __func__, addr);
        printk("%s: Read slave ram data %d bytes\n", __func__, ramlen);
...
static int jd9366tp_ReadChipID(uint8_t *rdata, uint16_t rlen)
{
...
static int jd9366tp_GetID(uint16_t *pRomID, uint32_t offset)
{
    ...
    uint8_t Drbuf[JD_FOUR_SIZE];

    ReCode = jd9366tp_ReadChipID(Drbuf, sizeof(Drbuf));
    printk("%s: %02x, %02x, %02x, %02x\n", __func__, Drbuf[0], Drbuf[1], Drbuf[2], Drbuf[3]);
    ...
    *pRomID = (uint16_t)((rbuf[1] << 8) + rbuf[0]);
    ...
    if (*pRomID == JD9366TP_ID) {
        if (JD9366TP_SLAVE_ADDR_OFFSET == offset)
            printk("%s: Slave IC ID: %04x\n", __func__, *pRomID);
        else
            printk("%s: Master IC ID: %04x\n", __func__, *pRomID);
    } else {
        if (JD9366TP_SLAVE_ADDR_OFFSET == offset)
...
static int jd9366tp_EnterBackDoor(uint16_t *pRomID)
{
    ...
    ReCode = jadard_bus_write(addrBuf, sizeof(addrBuf), writeBuf, sizeof(writeBuf), JD9366TP_BUS_RETRY_TIMES);
    ...
    if (jd9366tp_GetSectionFirstValueStatus(50) == true) {
        ...
        if (ReCode < 0) {
            ...
            if (ReCode < 0) {
                ...
                printk("%s: Section info was ready\n", __func__);
            }
        }
    } else {
...
static int jd9366tp_JEDEC_ID(uint8_t *id)
{
    ...
    printk("Flash Status1: 0x%02x ,0x%02x\n", rdbuf[0], rdbuf[1]);
    ...
    printk("Flash Status2: 0x%02x ,0x%02x\n", rdbuf[0], rdbuf[1]);
...
static int jd9366tp_HostWriteFlash(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
    ...
    /* Wait flash reload to Pram & PorInit time */
    msleep(1000);
...
static int jd9366tp_WritePram(uint32_t addr, uint8_t *pData, uint32_t len)
{
    ...
    /* Write start */
    if (len > 0) {
        printk("%s: Start write ram addr: 0x%08x\n", __func__, ramaddr);
        printk("%s: Write pram data %d bytes\n", __func__, ramlen);
    ...
    if (pjadard_ts_data->power_on_upgrade == false) {
        ...
        if (pjadard_ic_data->JD_MODULE_CASCADE_MODE == JD_CASCADE_MODE_ENABLE) {
            ...
            if (crc != MoveInfo[MoveInfoPsramIndex].to_mem_crc) {
                printk("%s: [Check slave ram]Binary crc is %04x, but read IC crc is %04x\n", __func__, MoveInfo[MoveInfoPsramIndex].to_mem_crc, crc);
                pram_fail = true;
            } else {
                printk("%s: [Check slave ram]HW CRC check pass\n", __func__);
            }
        }
        ...
        if (pram_fail == false) {
            ...
            /* Check IC crc */
            if (crc != MoveInfo[MoveInfoPsramIndex].to_mem_crc) {
                printk("%s: [Check master ram]Binary crc is %04x, but read IC crc is %04x\n", __func__, MoveInfo[MoveInfoPsramIndex].to_mem_crc, crc);
            } else {
                printk("%s: [Check master ram]HW CRC check pass, bypass upgrade flow\n", __func__);
...
GOTO_UPGRADE_FW:
    ...
    for (i = 0; i < MoveInfoNumber; i++) {
        ...
        if (softcrc != MoveInfo[i].to_mem_crc) {
            ...
            printk("%s: Software CRC check pass\n", __func__);
        }
    }
            ...
            if (crc != MoveInfo[MoveInfoPsramIndex].to_mem_crc) {
                ...
                if (pjadard_report_data->crc_start == 1) {
                    if (pjadard_report_data->crc_retry >= 2) {
                        ...
                        printk("%s: Upgrade slave\n", __func__);
                    } else {
                        pjadard_report_data->crc_retry++;
                        printk("%s: Recheck slave CRC\n", __func__);
                        msleep(500);
        ...
        if (pram_fail == false) {
            ...
            if (crc != MoveInfo[MoveInfoPsramIndex].to_mem_crc) {
                ...
                if (pjadard_report_data->crc_start == 1) {
                    if (pjadard_report_data->crc_retry >= 2) {
                        pjadard_report_data->crc_retry = 0;
                        printk("%s: Upgrade master\n", __func__);
                    } else {
                        pjadard_report_data->crc_retry++;
                        printk("%s: Recheck master CRC\n", __func__);
                        msleep(500);
...
static int jd9366tp_ReadPram(uint32_t addr, uint8_t *pData, uint32_t len)
{
    ...
    /* Read start */
    if (len > 0) {
        printk("%s: Start read ram addr: 0x%08x\n", __func__, ramaddr);
        printk("%s: Read ram data %d bytes\n", __func__, ramlen);
    ...
    if (ReadLen & 0xFFFF0000) {
        ...
        if (((ReadAddr & JD_S_DBI_DDREG_BASE_ADDR) == JD_S_DBI_DDREG_BASE_ADDR) ||
            ...
            if ((g_jd9366tp_chip_info.tcb_bus_speed > 0) &&
                ...
                printk("%s: Read slave DD register one by one\n", __func__);
                for (i = 0; i < ReadLen; i++) {
                    ReCode = jd9366tp_ReadRegSingle(ReadAddr + i, ReadData + i);
                }
            } else {
                printk("%s: Read slave DD register\n", __func__);
                pjadard_ic_data->JD_DISABLE_MASTER_TO_SLAVE = true;
...
{
    if (g_jadard_fw != NULL) {
        printk("Catch fw version in bin file\n");
...
static int module_jd9366tp_get_mutual_data(uint8_t data_type, uint8_t *rdata, uint16_t rlen)
{
    ...
    do {
        ...
        if (DataCnt++ > JD9366TP_DATA_TIMEOUT) {
            printk("Scan data timeout, DataCnt = %d\n", DataCnt);
            jd9366tp_EnterBackDoor(NULL);
            return JD_TIME_OUT;
        }
...
static bool module_jd9366tp_chip_detect(void)
{
    ...
    do {
        ...
        if (romid == CHIP_ID) {
            printk("%s: detect IC %s successfully\n", __func__, JD9366TP_OUTER_ID);
...
static int jadard_jd9366tp_probe(struct platform_device *pdev)
{
...
static int jadard_jd9366tp_remove(struct platform_device *pdev)
{
...
static int __init jadard_jd9366tp_init(void)
{
    printk("%s\n", __func__);
    platform_driver_register(&jadard_jd9366tp_driver);

    return 0;
}

static void __exit jadard_jd9366tp_exit(void)
{
    platform_driver_unregister(&jadard_jd9366tp_driver);
}
...
module_init(jadard_jd9366tp_init);

...
static bool jadard_ts_diag_func(void)
{
    ...
    if ((KeepFrame > 0) && (KeepFrame > jd_diag_mutual_cnt)) {
        ...
        if (!IS_ERR(jd_diag_mutual_fn)) {
            ...
            printk("Cancel write keep rawdata file for GKI\n");
#else
                                    ...
                                    size_t len, loff_t *data)
{
    ...
    printk("%s: DataType:%d, KeepType:%d, KeepFrame:%d\n", __func__, DataType, KeepType, KeepFrame);

#ifndef JD_KEEP_RAWDATA_DISABLE
    if (KeepFrame > 0) {
        printk("%s: Keep rawdata was disable, please using JDMT APK\n", __func__);
        return len;
    }
#endif
    ...
    if ((DataType > 0) && (KeepType > 0)) {
        ...
        if (KeepFrame > 0) {
            ...
            case JD_DATA_TYPE_LAPLACE:
                ...
                break;
            default:
                printk("%s: Data type is not support. DataType is %d \n", __func__, DataType);
            }
        }
#endif
        ...
        printk("%s: Start get mutual data\n", __func__);
...
{
#if 1 //defined(__JADARD_GKI__)
    printk("Cancel dump fw bin for GKI\n");
#else
    struct file *fn;
    uint32_t read_len;
    ...
    for (i = 0; i < chip_num; i++) {
...
#endif
        printk("%s: Read FW from chip complete\n", __func__);
        ...
        if (!IS_ERR(fn)) {
            printk("%s: create file and ready to write\n", __func__);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
                                            ...
                                            size_t len, loff_t *pos)
{
    ...
    printk("%s: buf_tmp = %s", __func__, buf_tmp);
    ...
    if ((buf_tmp[0] == 'S') && (buf_tmp[1] == 'T') && (buf_tmp[2] == 'D')) {
        ...
        if (pjadard_ts_data->dbi_std_mode_enable) {
            printk("Dbi std mode enable\n");
        } else {
            printk("Dbi std mode disable\n");
        }
    } else {
    ...
    if (!pjadard_debug->proc_send_flag) {
        ...
        if (buf_tmp != NULL) {
            if (debug_cmd == 'v') {
                ...
                if (pjadard_ic_data->printkNT_EDGE) {
                                 ...
                                 size_t len, loff_t *pos)
{
    ...
    if (buf_tmp[0] == 'v') {
        ...
        if (ret < 0) {
            fw_upgrade_complete = false;
            printk("Zero flash upgrade fail\n");
        } else {
        ...
        if (ret < 0) {
            printk("Fail to open file: %s (ret:%d)\n", fileName, ret);
            return ret;
        }
        ...
        if (pjadard_ic_data->JD_MODULE_CASCADE_MODE == JD_CASCADE_MODE_ENABLE) {
            ...
            if (pjadard_ic_data->JD_DISABLE_MASTER_TO_SLAVE) {
                printk("Direct read slave register\n");
            } else {
                printk("Read slave by master\n");
            }
        } else {
